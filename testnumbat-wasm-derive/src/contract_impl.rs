...
pub fn contract_implementation(
	...
	let main_definition = quote! {
		...
		testnumbat_wasm::api::ContractBase
		+ Sized
		#(#supertraits_main)*
		#where_self_big_int
		{
	...
	let endpoint_wrappers = quote! {
		pub trait EndpointWrappers:
			testnumbat_wasm::api::ContractPrivateApi
			+ #trait_name_ident
		...
		#where_self_big_int
		{
			...
			fn callback_selector<'a>(&self, mut ___cb_data_deserializer___: testnumbat_wasm::hex_call_data::HexCallDataDeserializer<'a>) -> testnumbat_wasm::types::CallbackSelectorResult<'a> {
