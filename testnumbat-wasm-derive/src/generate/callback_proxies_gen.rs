		...
		.filter_map(|m| {
			if let PublicRole::Callback(callback) = &m.public_role {
				...
				let proxy_decl = quote! {
					fn #method_name (self, #(#arg_decl),* ) -> testnumbat_wasm::types::CallbackCall{
						let ___api___ = self.into_api();
						let mut ___closure_arg_buffer___ = testnumbat_wasm::types::ArgBuffer::new();
						#(#arg_push_snippets)*
						testnumbat_wasm::types::CallbackCall::from_arg_buffer(#cb_name_literal, &___closure_arg_buffer___)
					}
