...
pub fn generate_event_impl(m: &Method, event_identifier: &str) -> proc_macro2::TokenStream {
	...
	let write_log_snippet = if let Some(data_arg) = data_arg {
		...
		quote! {
			let ___data_bytes___ = testnumbat_wasm::log_util::serialize_log_data(#data_pat, self.log_api_raw());
	...
	quote! {
		#msig {
			let mut ___topic_buffer___ = testnumbat_wasm::types::ArgBuffer::new();
