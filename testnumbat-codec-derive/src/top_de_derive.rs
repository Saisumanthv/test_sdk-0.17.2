		...
		syn::Data::Enum(data_enum) => {
			...
			if is_fieldless_enum(&data_enum) {
				...
				let top_decode_body = quote! {
					let bytes = top_input.into_boxed_slice_u8();
					let input = &mut &*bytes;
					let result = match <u8 as testnumbat_codec::NestedDecode>::dep_decode(input)? {
				...
				let top_decode_or_exit_body = quote! {
					let bytes = top_input.into_boxed_slice_u8();
					let input = &mut &*bytes;
					let result = match <u8 as testnumbat_codec::NestedDecode>::dep_decode_or_exit(input, c.clone(), exit) {
