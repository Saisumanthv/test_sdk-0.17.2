...
pub fn dep_decode_snippet(_index: usize, field: &syn::Field) -> proc_macro2::TokenStream {
	...
	if let Some(ident) = &field.ident {
		quote! {
			#ident: <#ty as testnumbat_codec::NestedDecode>::dep_decode(input)?
		}
	} else {
		quote! {
			<#ty as testnumbat_codec::NestedDecode>::dep_decode(input)?
		}
	}
}
...
pub fn dep_decode_or_exit_snippet(_index: usize, field: &syn::Field) -> proc_macro2::TokenStream {
	...
	if let Some(ident) = &field.ident {
		quote! {
			#ident: <#ty as testnumbat_codec::NestedDecode>::dep_decode_or_exit(input, c.clone(), exit)
		}
	} else {
		quote! {
			<#ty as testnumbat_codec::NestedDecode>::dep_decode_or_exit(input, c.clone(), exit)
		}
	}
}
